...
#================
#Plot for analysis of spatial locations
plot(df$lon,df$lat)
points(cmfd$lon,cmfd$lat)
...
library(ggpubr)
df1<-ggplot()+
  geom_point(data=df, aes(x=lon,y=lat,color='CMIP5-RCM Points'),pch=3,size=3.5)+
  geom_point(data=cmfd,aes(x=lon,y=lat,color='CMFD Pixel Centers'),pch=15)+
  ...
  geom_polygon(data=wanzhou_df, aes(long,lat, group=group), fill=NA, color="black",size=.5)+
  coord_sf(xlim = c(107.8,109), ylim = c(30.3,31.1)) +
  labs(title='RCM Points and CMFD Observations',x="Longitude",y="Latitude",color="Legend",fill="Legend") +
  theme_pubr()
df1
...
list.save(main.list,'ts.mpi-esm.rdata')
# list.save(rcm.list,'ts.mpi-esm.rdata')



#Part 1C: Partitioning Training and Validation Lists==========

load('ts.mpi-esm.rdata')
rcm.list<-x
x<-NULL
...
list.save(main.list,'ds.mpi-esm.rdata')

#-- Anaylsis
load('ds.mpi-esm.rdata')
ds.list<-x
