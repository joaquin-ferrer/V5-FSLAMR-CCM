...
load('day_sc_bc_mpiesm.rdata')
day.sc<-x
load('day_val_bc_mpiesm.rdata')
day.ref<-x
...
load('ext_sc_bc_mpiesm.rdata')
ext.sc<-x
load('ext_val_bc_mpiesm.rdata')
ext.ref<-x
...
ggplot()+
  ...
  labs(title=paste("Return Period Curves\nGCM-RCM: MPI-ESM-REMO"),x="Return Period [years]",y="Precipitation [mm/day]",color="Scenario")+
  theme_pubr()
