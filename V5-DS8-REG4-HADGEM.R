...
#============================================================
f=1 # f index := file this is  a trial for bigger funtion 
#============================================================


#1>>Open List of CCM Files -------------
files <- list()
files = list.files('/Volumes/DAT/2_code/data_regcm4',pattern='*.nc',full.names=TRUE)
files

#1>>Load List of observation list files (CMFD) --------
# 
# 
# #Script for xpanded extent..
# a.files <- list()
# a.files = list.files('/Volumes/DAT/2_code/data',pattern='*.nc',full.names=TRUE)
# a.files
# cmfd <- raster::brick(a.files[3])
# aoi <- raster::extent(107,110,30,31.5)
# 
# crop.wan<-crop(cmfd,aoi)
...
# }
# 
# list.save(main.list,'wan-rcm-expanded-ts.rdata')
# 

load('wan-extent-ts.rdata')
obs.ts <-x
# aa<-data.table::rbindlist(Map(c,lapply(x, '[', 'lon'),lapply(x, '[', 'lat')))
# plot(aa)
# lines(wanzhou_df,col='red')
...
list.save(main.list,'ts.reg4hadgem.rdata')

load('ts.reg4hadgem.rdata')
aa<-data.table::rbindlist(Map(c,lapply(x, '[', 'lon'),lapply(x, '[', 'lat')))
plot(aa)
...
list.save(main.list,'ds.reg4hadgem.rdata')
aa<-data.table::rbindlist(Map(c,lapply(main.list, '[', 'lon'),lapply(main.list, '[', 'lat')))

plot(aa$lon,aa$lat)
# #-- Anaylsis
# load('ds.reg4hadgem.rdata')
# ds.list<-x
# !=================!
# PERFORM SWEEP TEST!
# !=================!
...
for (r in 1:length(daysc.bc.list))
{
  ...
  #proj<-proj[proj$pr.d>=1,]
  proj<-mjja(proj)
  proj$pr.d[proj$pr.d<=0.001]<-0.001
  #Empirical CDF QDM
...
list.save(daysc.bc.list,'day_sc_bc_reg4hadgem.rdata')
...
list.save(sc.bc.list,'ext_sc_bc_reg4hadgem.rdata')
#}